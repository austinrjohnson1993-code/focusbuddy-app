    ...
    <div style={{ padding: '12px 14px', overflowY: 'auto', height: '100%', paddingBottom: 80 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '10px 12px', background: '#FF664412', border: '1px solid #FF664425', borderRadius: 6, cursor: 'pointer' }}>
          ...
          <span style={{ fontSize: 14, color: '#FF6644', fontFamily: "'Figtree',sans-serif", fontWeight: 500 }}>Add</span>
        </div>
      </div>
      ...
      {greet && (
        <div style={{ background: 'linear-gradient(135deg,#3E3228 0%,#211A14 100%)', borderRadius: 10, padding: '11px 12px', marginBottom: 10, border: '.5px solid #F5F0E318', position: 'relative', animation: 'greetIn .6s ease forwards' }}>
          ...
          <div style={{ animation: 'greetText .4s ease .4s both', fontSize: 14, fontWeight: 600, color: '#F5F0E3', fontFamily: "'Figtree',sans-serif", marginBottom: 3 }}>{greeting}, {name}</div>
          <div style={{ animation: 'greetText .4s ease .55s both', fontSize: 14, color: '#F5F0E390', fontFamily: "'Figtree',sans-serif", lineHeight: 1.5, paddingRight: 18 }}>
      ...
      {active.find(t => t.starred) && (() => { const p = active.find(t => t.starred); return (
        <div style={{ background: '#3E3228', borderRadius: 10, padding: 12, marginBottom: 10, borderLeft: '3px solid #E8321A' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
            <span style={{ fontSize: 14, textTransform: 'uppercase', letterSpacing: '.08em', color: '#E8321A', fontFamily: "'Figtree',sans-serif" }}>Do this first</span>
          </div>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#F5F0E3', fontFamily: "'Figtree',sans-serif" }}>{p.title}</div>
          {p.scheduled_for && <div style={{ fontSize: 14, color: '#F5F0E350', fontFamily: "'Figtree',sans-serif", marginTop: 3 }}>{new Date(p.scheduled_for).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</div>}
      ...
      {/* Loading state */}
      {loading && <div style={{ textAlign: 'center', padding: 20, fontSize: 14, color: '#F5F0E350', fontFamily: "'Figtree',sans-serif" }}>Loading tasks...</div>}

      {/* Task list */}
      <div style={{ fontSize: 14, textTransform: 'uppercase', letterSpacing: '.08em', color: '#F5F0E350', fontFamily: "'Figtree',sans-serif", fontWeight: 500, padding: '8px 0 4px' }}>Up next ({active.length})</div>
      {active.map(t => (
        <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 10px', background: '#3E3228', borderRadius: 8, marginBottom: 5, position: 'relative' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {t.scheduled_for && <div style={{ fontSize: 14, color: '#F5F0E350', fontFamily: "'Figtree',sans-serif", marginTop: 2 }}>{new Date(t.scheduled_for).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</div>}
          </div>
          ...
          {xpId === t.id && <div style={{ position: 'absolute', top: -4, right: 12, fontFamily: "'Sora',sans-serif", fontSize: 14, fontWeight: 600, color: '#FFB800', animation: 'xpFloat 1.2s ease forwards', pointerEvents: 'none' }}>+10 XP</div>}
        </div>
      ))}

      {/* Completed */}
      {done.length > 0 && <>
        <div style={{ fontSize: 14, textTransform: 'uppercase', letterSpacing: '.08em', color: '#F5F0E350', fontFamily: "'Figtree',sans-serif", fontWeight: 500, padding: '8px 0 4px' }}>Completed ({done.length})</div>
        {done.map(t => (
          <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 10px', background: '#3E3228', borderRadius: 8, marginBottom: 5, opacity: 0.45 }}>
            ...
            <span style={{ fontSize: 14, color: '#F5F0E350', fontFamily: "'Figtree',sans-serif", textDecoration: 'line-through' }}>{t.title}</span>
          </div>
        ))}
      </>}
      ...
      <div style={{ background: '#3E3228', borderRadius: 8, padding: '8px 10px', marginTop: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <span style={{ fontFamily: "'Sora',sans-serif", fontSize: 14, fontWeight: 600, color: '#FF6644' }}>{done.length}/{tasks.filter(t => !t.archived).length}</span>
            <span style={{ fontSize: 14, color: '#F5F0E350', fontFamily: "'Figtree',sans-serif" }}>today</span>
          </div>
          <span style={{ fontSize: 14, color: '#FFB800', fontFamily: "'Sora',sans-serif", fontWeight: 500 }}>{profile?.total_xp || 0} XP &middot; {profile?.current_streak || 0}d streak</span>
        </div>
