...
export function buildPersonaPrompt(profile) {
  ...
  const buildHabits = habits.filter(h => h.habit_type === 'build').map(h => h.name)
  const breakHabits = habits.filter(h => h.habit_type === 'break').map(h => h.name)
  const habitsLine = (buildHabits.length > 0 || breakHabits.length > 0)
    ? [
        buildHabits.length > 0 ? `- User is building these habits: ${buildHabits.join(', ')}. Celebrate streaks and momentum on these.` : null,
        breakHabits.length > 0 ? `- User is trying to break these habits: ${breakHabits.join(', ')}. Celebrate resistance and clean days on these.` : null,
      ].filter(Boolean).join(' ')
    : ''
